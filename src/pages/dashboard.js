...
<div style="background:radial-gradient(ellipse 420px 220px at 92% -20%,rgba(34,211,238,0.08),transparent 60%),radial-gradient(ellipse 380px 200px at 8% 120%,rgba(34,197,94,0.07),transparent 60%),linear-gradient(135deg,var(--bg-elevated),var(--bg-card));border:1px solid var(--border-mid);border-radius:var(--radius-lg);padding:20px 24px;margin-bottom:16px;box-shadow:inset 0 1px 0 rgba(255,255,255,0.04);">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:14px;">
    <div class="stat-chip"><span class="chip-icon">✅</span><span class="chip-val">${todayTasks.filter(t=>t.done).length}/${todayTasks.length}</span><span class="chip-lbl">Tasks Today</span></div>
    <div class="stat-chip"><span class="chip-icon">🔥</span><span class="chip-val">${doneHabits}/${(DB.habits||[]).length}</span><span class="chip-lbl">Habits Done</span></div>
    <div class="stat-chip"><span class="chip-icon">🎯</span><span class="chip-val">${activeGoals}</span><span class="chip-lbl">Active Goals</span></div>
    <div class="stat-chip"><span class="chip-icon">🚀</span><span class="chip-val">${activeProjs}</span><span class="chip-lbl">Projects</span></div>
    <div class="stat-chip"><span class="chip-icon">💰</span><span class="chip-val" style="font-size:13px;">${pkr(bal)}</span><span class="chip-lbl">Balance</span></div>
  </div>
</div>

<div class="grid grid-cols-2 gap-3 max-[780px]:grid-cols-1" style="margin-bottom:12px;">
  <div class="card">
    <div class="card-header"><span class="card-title">Time Remaining</span></div>
    <div id="dash-countdown"></div>
  </div>
  <div class="card">
    <div class="card-header"><span class="card-title">Focus — Last 7 Days</span><span style="font-size:11px;font-family:var(--font-mono);color:var(--accent);">${weekTotal}h</span></div>
